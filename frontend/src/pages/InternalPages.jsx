...
export const CaseExplorer = () => (
  <div className="bg-[#0A0A0A] min-h-screen pt-32">
    ...
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="glass-card p-6 mb-8 flex flex-wrap gap-4 items-center">
        <div className="flex-1 min-w-[300px] relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
          <input type="text" placeholder="Search precedents by keyword, judge, or citation..." className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-3 text-white" />
        </div>
        <button className="btn-secondary flex items-center gap-2 text-sm"><Filter size={16} /> Filters</button>
        <button className="btn-secondary flex items-center gap-2 text-sm"><Download size={16} /> Export List</button>
      </div>
          ...
          <tbody className="text-sm text-slate-300 divide-y divide-white/5">
            {[1, 2, 3, 4, 5].map(i => (
              <tr key={i} className="hover:bg-white/5 transition-colors cursor-pointer">
                <td className="px-6 py-4 font-mono text-orange-500/80">202{i} SC {120 + i}</td>
                ...
                <td className="px-6 py-4"><span className="text-green-500 bg-green-500/10 px-2 py-0.5 rounded text-[10px] font-bold">Valid</span></td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  <div className="bg-[#0A0A0A] min-h-screen pt-32">
    ...
    <div className="max-w-4xl mx-auto px-6">
       ...
       <div className="space-y-4">
         {[1,2,3].map(i => (
           <div key={i} className="glass-card p-6 flex justify-between items-center hover:border-orange-500/20 transition-all cursor-pointer group">
             <div>
               <h3 className="font-serif text-lg mb-1 group-hover:text-orange-400 transition-colors">Research on PMLA Section 45 Bail</h3>
               <p className="text-xs text-slate-500">Generated on May {i+1}, 2026 • 12 Verified Citations</p>
             </div>
             <button className="text-slate-500 hover:text-white"><Download size={18} /></button>
           </div>
         ))}
       </div>
    </div>
