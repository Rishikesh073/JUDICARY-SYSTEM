...
const LiveWorkspace = () => {
  ...
  return (
    <section className="py-24 px-6 bg-black/20" id="research">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-xs uppercase tracking-widest text-orange-500 font-bold mb-4 block">Live Workspace</span>
          <h2 className="text-4xl font-serif mb-4 text-white">Ask. Analyze. Act.</h2>
          <p className="text-slate-500">Natural language legal queries — no boolean operators, no keyword games.</p>
        </div>

        <div className="glass-card p-8 bg-black/40 border-white/5 shadow-2xl">
          <div className="mb-10">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-4 text-white focus:outline-none focus:border-orange-500/50 transition-all placeholder:text-slate-600 italic"
              />
              <button 
            ...
            <div className="flex gap-2 mt-4">
              {['Supreme Court', '2014-2025', 'PMLA'].map(tag => (
                <span key={tag} className="px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[10px] text-slate-500 font-bold">{tag}</span>
              ))}
            </div>
          </div>
              ...
              {STEPS.map((step, idx) => (
                <div key={step.id} className="flex flex-col items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold transition-all border-2 ${
                    ...
                    currentStep === idx + 1 ? 'bg-black border-orange-600 text-orange-500 shadow-[0_0_15px_rgba(217,119,6,0.3)]' :
          ...
          <AnimatePresence>
            {memo && (
              <motion.div 
                ...
                className="bg-[#0F0F12] border border-white/5 rounded-2xl p-8"
              >
                <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-serif text-white">Legal memorandum — {memo.query.substring(0, 30)}...</h3>
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="bg-white/5 hover:bg-white/10 text-slate-300 text-[10px] font-bold px-4 py-2 rounded-lg border border-white/10 flex items-center gap-2 transition-all"
                    >
                      <Download size={14} /> Download PDF
                    </button>
                    <span className="bg-green-500/10 text-green-400 text-[10px] font-bold px-3 py-1 rounded-full border border-green-500/20 flex items-center gap-1">
                      <CheckCircle2 size={12} /> Verified
                    </span>
                  </div>
                </div>

                <div className="space-y-8 text-slate-400 leading-relaxed max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
          ...
          {error && (
            <div className="mt-8 p-4 bg-red-900/10 border border-red-500/20 rounded-xl text-red-400 text-sm text-center">
              {error}
            </div>
          )}
        </div>
